	...
	for cy in range(int(grid_size.y) + 2):
		for cx in range(int(grid_size.x) + 2):
			if cx % 2 and cy % 2:
				...
				# X only is odd then is a slope in the Z direction
				draw_y_slope(surf_tool, cell_delta, cell_scale, data, (cx - 1) / 2, cy / 2)
			elif cy % 2:
				# X only is odd then is a slope in the Z direction
...
func draw_y_slope(surf_tool, cell_delta, cell_scale, data, data_x, data_y):
